...
export type NetworkNode = { id: string; x: number; y: number; population: number; destinations: number }
...
export type RouteResult = { edgeIds: string[]; nodeIds: string[]; minutes: number; highStressEdges: number; path: string }
...
export function validateSimulationNetwork(network: SimulationNetwork): string[] {
  ...
  const unique = (ids: string[]) => ids.every((id) => typeof id === 'string' && /^[a-zA-Z0-9][a-zA-Z0-9._:-]*$/.test(id)) && new Set(ids).size === ids.length
  ...
  for (const node of network.nodes) if (![node.x, node.y].every(Number.isFinite) || ![node.population, node.destinations].every((n) => validNumber(n) && Number.isSafeInteger(n))) errors.push(`Invalid node ${node.id}`)
  ...
  for (const edge of network.edges) if (!ids.has(edge.from) || !ids.has(edge.to) || edge.from === edge.to || !validNumber(edge.minutes) || edge.minutes === 0 || typeof edge.highStress !== 'boolean' || (edge.corridorId !== undefined && (typeof edge.corridorId !== 'string' || !/^[a-zA-Z0-9][a-zA-Z0-9._:-]*$/.test(edge.corridorId)))) errors.push(`Invalid edge ${edge.id}`)
  ...
  for (const flow of network.flows) if (!ids.has(flow.from) || !ids.has(flow.to) || flow.from === flow.to || !validNumber(flow.weight) || typeof flow.corridorId !== 'string' || !/^[a-zA-Z0-9][a-zA-Z0-9._:-]*$/.test(flow.corridorId)) errors.push(`Invalid flow ${flow.id}`)
...
/** Deterministic Dijkstra, low-stress-only first, stress-penalized fallback second. */
function route(network: SimulationNetwork, from: string, to: string, active: Set<string>, lowStressOnly: boolean): RouteResult | null {
  ...
  while (remaining.size) {
    ...
    for (const edge of edges) {
      ...
      const candidate = distance.get(next)! + edge.minutes * (stressed(edge) ? network.stressPenalty : 1)
  ...
  return {
    ...
    path: nodeIds.map((id, index) => `${index ? 'L' : 'M'} ${nodes.get(id)!.x} ${nodes.get(id)!.y}`).join(' '),
  }
}

function safeRoute(network: SimulationNetwork, flow: DemandFlow, active: Set<string>): RouteResult | null {
  return route(network, flow.from, flow.to, active, true) ?? route(network, flow.from, flow.to, active, false)
}
...
export function simulateDemandNetwork(corridor: Corridor, artifact: CorridorDemandArtifact, network: SimulationNetwork, state: PortfolioNetworkState): CorridorSimulation {
  ...
  const warnings = ['Scenario assumptions: bidirectional edges; construction converts tagged edges to low stress without changing travel time. Accessibility uses a low-stress travel-time threshold.']
  ...
  if (routes.some((item) => item.after && item.after.highStressEdges > 0)) warnings.push('Some OD pairs have no fully low-stress alternative; their routes retain high-stress edges.')
  return {
